// Mock API server that returns fake product reviews for local testing.
import express from 'express';

const app = express();
const PORT = 3001;

// Mock review data for two products
const mockData: Record<string, { productId: string; productName: string; reviews: any[] }> = {
  'product-a': {
    productId: 'product-a',
...
app.listen(PORT, () => {
  ...
  console.log(`  GET http://localhost:${PORT}/products/product-a/reviews`);
  console.log(`  GET http://localhost:${PORT}/products/product-b/reviews`);
});
